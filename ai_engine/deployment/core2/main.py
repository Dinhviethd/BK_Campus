...
# Đảm bảo bạn đã upload file origin_clip.py lên Files của Space
try:
    from origin_clip import ClipEmbedder
except ImportError:
    print("WARNING: Không tìm thấy file origin_clip.py. Hãy upload nó lên HF Space.")
    ClipEmbedder = None # Xử lý tạm để không crash app ngay lập tức

# 1. Cấu hình Environment & Khởi tạo App
load_dotenv()
app = FastAPI()

# Lấy biến môi trường từ HF Secrets
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_KEY")
GROQ_API_KEY = os.getenv("GROQ_API_KEY")
API_SECRET = os.getenv("API_SECRET") # Biến mới để bảo mật webhook
...
# 3. Khởi tạo Model CLIP (Load 1 lần khi server start)
print("--- KHỞI TẠO HỆ THỐNG AI ---")
if ClipEmbedder:
    clip_embedder = ClipEmbedder()
else:
    clip_embedder = None
    print("Error: ClipEmbedder chưa được load.")

MODEL_ID = "meta-llama/llama-4-scout-17b-16e-instruct" # Cập nhật model Vision ổn định

# --- CÁC HÀM XỬ LÝ LOGIC GỐC (GIỮ NGUYÊN) ---

def fetch_pending_posts():
    """Lấy các bài viết đang ở trạng thái PROCESSING [cite: 8]"""
...
def get_post_images(post_id):
    """Lấy danh sách URL ảnh từ bảng post_images [cite: 9]"""
...
def process_logic_task():
    """Hàm chạy ngầm xử lý toàn bộ logic cũ"""
    if not clip_embedder:
        print("Model CLIP chưa sẵn sàng. Bỏ qua.")
        return
    ...
    for post in posts:
        try:
            ...
            if result:
                ...
                # Logic xác định trạng thái [cite: 8]
                ai_status = "ACTIVE"
                if ai_type == "TRASH":
                    ai_status = "REJECTED"
                ...
                # BƯỚC 2: Embedding (Nếu ACTIVE)
                if ai_status == "ACTIVE":
                    # 2.1 Embed Text (Sửa lại size vector nếu cần theo SQL edit: vector(512) hay 1536)
                    # Lưu ý: Code gốc đang dùng model CLIP nào thì ra size đó.
...
@app.get("/")
def health_check():
    return {"status": "ok", "message": "Core12 Fusion AI Service is running"}
