...
def run_crawler_pipeline():
    ...
    with sync_playwright() as p:
        # 2. Đăng nhập với Proxy và FacebookSession
        browser = p.chromium.launch(
            headless=True, 
            args=["--disable-notifications", "--no-sandbox"],
            proxy=proxy_config # <--- Dùng Proxy khi đăng nhập
        )
        ...
        # --- [MỚI] DÙNG CLASS FACEBOOKSESSION ---
        fb_session = FacebookSession(STATE, FB_EMAIL, FB_PASS)
        login_success = fb_session.login(page, context)
        
        browser.close() 
        
        if not login_success:
            print("!!! ❌ Đăng nhập thất bại. Hủy Job.")
            return

    # 3. Khởi tạo Threading (Truyền proxy_config vào worker)
    # Lưu ý: args phải là tuple, nên cần dấu phẩy (proxy_config,)
    t1 = threading.Thread(target=collect_links_worker, args=(proxy_config,), daemon=True)
    ...
    print(f"[{time.strftime('%X')}] >>> ✅ HOÀN TẤT JOB CRAWL.")
...
def collect_links_worker(proxy_config):
    with sync_playwright() as p:
        browser = p.chromium.launch(
            headless=True, 
