...
def perform_login(page, context):
    ...
    try:
        print(">>> Test mạng: Đang thử vào Google...")
        page.goto("https://www.google.com", timeout=5000)
        print(">>> ✅ Mạng OK, Google vào được.")
    except Exception as e:
        print(f">>> ❌ Mất mạng hoàn toàn: {e}")
        return False
    ...
    try:
        ...
        # Thay dòng này
        # page.goto("https://m.facebook.com/", wait_until="domcontentloaded")
        # Bằng dòng này (Thử 1 trong 2)
        page.goto("https://www.facebook.com/", wait_until="domcontentloaded") 
        # page.goto("https://mbasic.facebook.com/", wait_until="domcontentloaded")
        time.sleep(10)
...
def scrape_feed_hook():
    ...
    with sync_playwright() as p:
        ...
        browser = p.chromium.launch(headless=True, channel="chrome", args=["--disable-notifications"])
        context = browser.new_context(
            storage_state=STATE if os.path.exists(STATE) else None,
            user_agent=USER_AGENT,
            viewport={'width': 375, 'height': 812},
...
# --- BỔ SUNG: HÀM ĐIỀU PHỐI CHÍNH (THAY CHO MAIN CŨ) ---
...
def run_crawler_pipeline():
    ...
    with sync_playwright() as p:
        # 2. Đăng nhập trước (tuần tự) để lấy session
        ...
        login_success = perform_login(page, context)
        browser.close() # Đóng để giải phóng resource, worker sẽ tự mở lại
        
        if not login_success:
            print("!!! ❌ Đăng nhập thất bại. Hủy Job.")
            return

    # 3. Khởi tạo Threading (Như logic cũ của bạn)
    ...
    # 5. Chờ cả 2 thread kết thúc (Blocking để background task không bị kill sớm)
    t1.join()
    t2.join()
