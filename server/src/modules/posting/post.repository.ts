...
import { UpdatePostDTO, CreatePostDTO } from '@/modules/posting/post.schema';
...
export class PostRepository {
  ...
  async create(data: CreatePostDTO): Promise<Post> {
    const post = this.repository.create({
      ...
      status: process_status.ingested,
  ...
  /** Xoá 1 ảnh */
  async removeImage(postId: string, imageId: string): Promise<boolean> {
    const result = await this.imageRepository.delete({ id: imageId, post: { id: postId } as any });
    return result.affected !== 0;
  }
  ...
  async updateTypeAndStatus(
    id: string,
    payload: { status: process_status; type?: post_type }
  ): Promise<Post | null> {
  ...
  async updateEmbeddingResult(
    id: string,
    payload: {
      status: process_status;
      extractedInfo?: Record<string, unknown>;
      itemTypeEmbedding?: number[];
      imageFeature?: {
        postImagesId: string;
        imageUrls: string[];
    ...
    await this.repository.update(id, {
      ...
      ...(payload.extractedInfo !== undefined
        ? { extractedInfo: payload.extractedInfo as any }
        : {}),
    ...
    if (payload.imageFeature) {
      await this.imageRepository.update(
        { id: payload.imageFeature.postImagesId, post: { id } as any },
        {
          url: payload.imageFeature.imageUrls,
          extractedFeatures:
            payload.imageFeature.extractedFeatures !== undefined
              ? (payload.imageFeature.extractedFeatures as any)
              : null,
        }
      );
    }
